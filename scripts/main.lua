...
--! configuration
local service = 5039;  -- your service id, this is used to identify your service.
local secret = "48ef5720-d0d7-443e-b96d-5b84b3bd7786";  -- obfuscate this if you want security.
...
--! pick host (fixed logic: use AND instead of OR)
...
if not success or (hostResponse.StatusCode ~= 200 and hostResponse.StatusCode ~= 429) then
...
local function cacheLink()
    if cachedTime + (10*60) < fOsTime() then
        ...
        if not ok or not response then
            local msg = "Failed to request start endpoint."
            onMessage(msg)
            return false, msg
        end
        ...
        if response.StatusCode == 200 then
            ...
            onMessage("Start endpoint returned status code "..tostring(response.StatusCode));
...
local function generateNonce()
    local nonce = "";
    for i = 1, 12 do
        nonce = nonce .. string.char(fMathRandom(33, 126));
    end
    return nonce;
end

local function hashNonce(nonce)
    return lDigest(secret .. nonce);
end
...
local function verifyKey(key)
    ...
    if response.StatusCode == 200 then
        ...
        if decoded.success == true then
            if useNonce and (decoded.nonce == nil or decoded.hash == nil) then
                onMessage("Missing nonce or hash in response.");
                return false;
            end

            if useNonce then
                local expectedHash = lDigest(secret .. decoded.nonce);
    ...
    elseif response.StatusCode == 429 then
        onMessage("Too many requests, please wait.");
        return false;
    else
        onMessage("Verification failed with status code " .. tostring(response.StatusCode));
        return false;
    end
end
...
local function redeemKey(key)
    ...
    elseif response.StatusCode == 429 then
        ...
        onMessage("Redemption failed with status code " .. tostring(response.StatusCode));
        return false;
    end
end

-- Example usage (just for illustration, remove or replace with your UI logic)
task.spawn(function()
    local ok, link = cacheLink()
    if ok then
        onMessage("Got link: " .. link)
    end
end)
